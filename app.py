...
class Notifications(Resource):
    ...
    def get(self):
        try:
            user_id = g.current_user['user_id']
            sql = """
                SELECT m.message_id, u.username, c.chatroom_name, c.chatroom_type, m.content, m.sent_at
                FROM messages m
                JOIN users u ON m.sender_id = u.user_id
                ...
                LIMIT 30;
            """
            rows = queryDB(sql, (user_id, user_id))

            notifications = []
            for row in rows:
                chatroom_type = row[3]
                title = 'New Direct Message' if chatroom_type == 'Direct Message' else 'New Group Message'
                notifications.append({
                    'id': str(row[0]),
                    'title': title,
                    'message': f'{row[1]}: {row[4]}',
                    'time': row[5].isoformat(),
                    'type': 'message' if chatroom_type == 'Direct Message' else 'group',
                    'chatroom_name': row[2],
                })

            return {'success': True, 'data': notifications}, 200
